...
<div class="box-container">
   ...
   <?php
     ...
     $sql = "SELECT * FROM `commande_client` where etat_commande='Completed'	";
     $stmt = $conn->query($sql);
     ...
     if ($rows) {
        foreach ($rows as $row) {
            ...
            echo '</form>';
            echo '<form action="reject_livreur.php" method="post" style="display:inline-block;">';
            echo '<input type="hidden" name="Id_commande" value="' . htmlspecialchars($row["id_commande"]) . '">';
            echo '<button class="reject" type="submit">Reject</button>';
            echo '</form>';
            echo '</div>';
            echo '<div>';
            echo '<form action="placed_orders.php" method="post">';
            echo '<input type="hidden" name="order_id" value="' . htmlspecialchars($row["id_commande"]) . '">';
            echo '<select name="payment_status" class="select">';
            echo '<option selected disabled>' . htmlspecialchars($row["etat_commande"]) . '</option>';
            echo '<option value="pending">Pending</option>';
            echo '<option value="completed">Completed</option>';
            echo '</select>';
            echo '<div class="flex-btn">';
            echo '<input type="submit" value="Update" class="option-btn" name="update_payment">';
            echo '<a href="placed_orders.php?delete=' . htmlspecialchars($row["id_commande"]) . '" class="delete-btn" onclick="return confirm(\'delete this order?\');">Delete</a>';
            echo '</div>';
            echo '</form>';
            echo '</div>';
            echo '</div>';
