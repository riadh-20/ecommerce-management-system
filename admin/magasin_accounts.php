...
<section class="accounts">
   ...
   <?php
      $select_accounts = $conn->prepare("SELECT * FROM `magasin`");
      $select_accounts->execute();
      if($select_accounts->rowCount() > 0){
         while($fetch_accounts = $select_accounts->fetch(PDO::FETCH_ASSOC)){   
   ?>
   <div class="box">
      <p> user id : <span><?= $fetch_accounts['Id_mag']; ?></span> </p>
      ...
      <a href="magasin_accounts.php?delete=<?= $fetch_C['id_compte']; ?>" onclick="return confirm('delete this account? the user related information will also be deleted!')" class="delete-btn">delete</a>
   </div>
   <?php
         }
      }else{
