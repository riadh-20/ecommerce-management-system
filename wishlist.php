...
if(isset($_GET['delete_all'])){
   $delete_wishlist_item = $conn->prepare("DELETE FROM `wishlist` WHERE ID_client = ?");
   $delete_wishlist_item->execute([$ID_client]);
...
<section class="products">
   ...
   <?php
      $grand_total = 0;
      $select_wishlist = $conn->prepare("SELECT * FROM `wishlist` WHERE ID_client = ?");
      $select_wishlist->execute([$ID_client]);
